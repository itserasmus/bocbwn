...
run:
    ...
    for(command& cmd : commands) {
        switch(cmd.opc) {
            case MOV: {
                am.push_instr(addx, quad_r, Loc(cmd.off), Loc(R14));
                am.push_instr(andx, quad_r, Loc(RBP), Loc(R14));
                break;}
            case ADD:
                am.push_instr(addx, byte_r, Loc(cmd.aux), Loc(cmd.off, RBX, R14));
                break;
            case OUT:
                am.push_instr(movzxq, byte_r, Loc(cmd.off, RBX, R14), Loc(AsmMan::argument_reg[0]));
            ...
            case IN:
                ...
                am.push_instr(movx, byte_r, Loc(AsmMan::return_reg[0]), Loc(cmd.off, RBX, R14));
                break;
            case BRZ:
                am.push_instr(cmpx, byte_r, Loc(0), Loc(cmd.off, RBX, R14));
                ...
                break;
            case BRNZ: {
                int32_t l_id = commands[cmd.match].match;
                am.push_instr(cmpx, byte_r, Loc(0), Loc(cmd.off, RBX, R14));
                am.push_instr(jnz, l_id);
                am.push_instr(label, l_id + 1);
                break;}
            case SET:
                am.push_instr(movx, byte_r, Loc(cmd.aux), Loc(cmd.off, RBX, R14));
                break;
            case PUTA:
                am.push_instr(movx, byte_r, Loc(cmd.off, RBX, R14), Loc(R12));
                break;
            case ACCUMA:
                am.push_instr(addx, byte_r, Loc(R12), Loc(cmd.off, RBX, R14));
                break;
            case MACMA:
                am.push_instr(imulx, byte_r, Loc(R12), Loc(RAX), cmd.aux, true);
                am.push_instr(addx, byte_r, Loc(RAX), Loc(cmd.off, RBX, R14));
                break;
            case RKILL:
                break;
            case NOP:
